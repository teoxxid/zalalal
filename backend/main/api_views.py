...
@extend_schema(
    responses={200: {"description": "Страница пользователя"}, 401: {"description": "Не авторизован"}},
)
@api_view(['GET'])
...
@extend_schema(
    responses={200: {"description": "Страница администратора"}, 401: {"description": "Не авторизован"}, 403: {"description": "Доступ запрещен"}},
)
@api_view(['GET'])
@permission_classes([IsAuthenticated])
...
def raw_add_to_cart(request):
    ...
    if request.method == 'POST':
        try:
            ...
            return JsonResponse({"status": "error", "message": "Услуга не найдена"}, status=404)
    ...
    if request.method == 'POST':
        try:
            ...
            if not all([name, price, description, category, brand]):
                return JsonResponse({"status": "error", "message": "Не заполнены обязательные поля"}, status=400)
            ...
            if "image" in request.FILES:
                ...
                if not success:
                    return JsonResponse({"status": "error", "message": "Ошибка загрузки изображения"}, status=500)
            ...
            if "video" in request.FILES:
                ...
                if not success:
                    return JsonResponse({"status": "error", "message": "Ошибка загрузки видео"}, status=500)
    ...
    if request.method == 'PUT':
        try:
            ...
            if order.user.id != user.id:
                return JsonResponse({"error": "Нельзя редактировать чужую заявку"}, status=403)
            
            if order.status != "draft":
                return JsonResponse({"error": "Редактировать можно только черновик"}, status=400)
...
def raw_delete_order_item(request, order_id, service_id):
    ...
    if request.method == 'DELETE':
        try:
            ...
            if order.user.id != user.id:
                return JsonResponse({"error": "Нельзя удалить из чужой заявки"}, status=403)
            
            order_item = get_object_or_404(OrderItem, order=order, service=service)
            order_item.delete()
            
            return JsonResponse({"status": "success", "message": "Услуга удалена из заявки"})
        except Exception as e:
            return JsonResponse({"error": str(e)}, status=400)
    ...
    if request.method == 'PUT':
        try:
            ...
            return JsonResponse({
                ...
                "data": {"id": order.id, "delivery_address": order.delivery_address}
            })
        except Exception as e:
            return JsonResponse({"error": str(e)}, status=400)
...
def raw_submit_order(request, order_id):
    ...
    if request.method == 'PUT':
        try:
            ...
            if order.user.id != user.id:
                return JsonResponse({"error": "Нельзя сформировать чужую заявку"}, status=403)
            
            if order.status != "draft":
                return JsonResponse({"error": "Сформировать можно только черновик"}, status=400)
            ...
            if not items.exists():
                return JsonResponse({"error": "Нельзя сформировать пустую заявку"}, status=400)
            
            total = sum(float(item.quantity) * float(item.price_at_time) for item in items)
...
def raw_complete_order(request, order_id):
    ...
    if request.method == 'PUT':
        try:
            ...
            if request.user.role != 'ADMIN':
                return JsonResponse({"error": "Только модератор может завершить заявку"}, status=403)
            
            if order.status != "submitted":
                return JsonResponse({"error": "Завершить можно только сформированную заявку"}, status=400)
...
def raw_reject_order(request, order_id):
    ...
    if request.method == 'PUT':
        try:
            ...
            if request.user.role != 'ADMIN':
                return JsonResponse({"error": "Только модератор может отклонить заявку"}, status=403)
            
            if order.status != "submitted":
                return JsonResponse({"error": "Отклонить можно только сформированную заявку"}, status=400)
...
def raw_delete_order(request, order_id):
    ...
    if request.method == 'DELETE':
        try:
            ...
            if order.user.id != user.id:
                return JsonResponse({"error": "Нельзя удалить чужую заявку"}, status=403)
            
            if order.status not in ["draft", "submitted", "completed"]:
                return JsonResponse({"error": "Нельзя удалить заявку в этом статусе"}, status=400)
