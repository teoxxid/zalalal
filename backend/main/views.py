...
from .models import Service, Order, OrderItem, User
...
def service_list(request):
    ...
    user, _ = User.objects.get_or_create(
        id=1,
        defaults={'username': 'user1', 'email': 'user1@mail.ru'}
    )
...
def service_detail(request, service_id):
    ...
    service.image_url = f"http://localhost:9000/services/{service.image_key}"
    service.video_url = f"http://localhost:9000/services/{service.video_key}" if service.video_key else None
...
def add_to_order(request, service_id):
    ...
    if request.method == 'POST':
        ...
        total = sum(item.quantity * item.price_at_time
...
def delete_order(request, order_id):
    ...
    if request.method == 'POST':
        ...
        with connection.cursor() as cursor:
            cursor.execute(
                "UPDATE main_order SET status = 'deleted' WHERE id = %s",
                [order_id]
            )
